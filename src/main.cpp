...
unsigned int nStakeMinAge = 8 * 60 * 60; // Moincoin - 8 hours
unsigned int nStakeMaxAge = -1; // Moincoin - unlimited
unsigned int nModifierInterval = 10 * 60; // Moincoin - time to elapse before new modifier is computed
...
const string strMessageMagic = "Moincoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight+1 >= 1 && pindexBest->nHeight+1 <= 19)  
    {    
        nSubsidy = 0 * COIN;
    }    
    else if(pindexBest->nHeight+1 >= 20 && pindexBest->nHeight+1 <= 999) 
    {    
        nSubsidy = 79 * COIN; 
    }    
    else if(pindexBest->nHeight+1 >= 1000 && pindexBest->nHeight+1 <= 17999 )
    {    
        nSubsidy = 59 * COIN; 
    }    
    else if(pindexBest->nHeight+1 >= 18000 && pindexBest->nHeight+1 <= 20000)
    {    
        nSubsidy = 89 * COIN; 
    }   
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Moincoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "North Korea says it can miniaturize nuclear weapons - Wed 20th May 2015 cnn.com";  
        CTransaction txNew;
        txNew.nTime = 1432123071;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1432123071;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   =  1057093;
     ...
     if (false && block.GetHash() != hashGenesisBlock) {
        ...
        assert(block.hashMerkleRoot == uint256("0x184d27e55f19ffc5232ec1a8d71751df0c2e8e0000979584782ea097e3a17613"));
...
unsigned char pchMessageStart[4] = { 0x9b, 0xac, 0x8f, 0xc9 };
